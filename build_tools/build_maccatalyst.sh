...
function build_fat_lib(){
    local lib_names=
    local searchLibs=$(find install/$1/maccatalyst -name *.a)
    ...
    local fat_dir=install/$1/maccatalyst/fat/lib
    rm -rf ${fat_dir}
    mkdir -p  ${fat_dir}
    for libname in ${lib_names}
    do
         libs=$(find install/$1/maccatalyst -name ${libname})
         lipo -create ${libs}  -output install/$1/maccatalyst/fat/lib/${libname}
    done
}
...
function build_shared_framework(){
    ...
    SRC_LIBRARIES="$(cd ./install/ffmpeg/maccatalyst/fat/lib; ls)"
    ...
    SRC_LIBRARIES_DIR="$CWD/install/ffmpeg/maccatalyst/fat/lib"
    SRC_INCLUDE_DIR="$CWD/install/ffmpeg/maccatalyst/arm64/include/"

    for support_lib in ${support_libs}
    do
        if [[ -d "install/${support_lib}/maccatalyst/fat/lib" ]];then
            SRC_LIBRARIES_DIR="$SRC_LIBRARIES_DIR $CWD/install/${support_lib}/maccatalyst/fat/lib"
            local libs="$(cd install/${support_lib}/maccatalyst/fat/lib; ls)"
            SRC_LIBRARIES="$SRC_LIBRARIES $libs"
        fi
    done

    if [[ -d "${DAV1D_EXTERNAL_DIR}/maccatalyst/fat" ]];then
        SRC_LIBRARIES_DIR="$SRC_LIBRARIES_DIR ${DAV1D_EXTERNAL_DIR}/maccatalyst/fat/lib"
        SRC_LIBRARIES="$SRC_LIBRARIES dav1d"
    fi

 #   $CWD/install/openssl/iOS/fat/lib"
    cd ./install/ffmpeg/maccatalyst/
    cp ${BUILD_TOOLS_DIR}/maccatalyst/CMakeLists.txt ./
...
build_libs maccatalyst "${MACCATALYST_ARCHS}"
build_fat_libs
